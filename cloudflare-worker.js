...
export default {
  ...
  async scheduled(event, env, ctx) {
    ctx.waitUntil((async () => {
      try {
        const now = new Date();
        const hour = now.getUTCHours(); // or local IST
        // Automated class checks can be triggered here with D1 database binding
        console.log('Sur Slot Cloudflare Worker Cron Trigger ran at:', now.toISOString());
      } catch (err) {
        console.error('Scheduled cron error in worker:', err);
      }
    })());
  }
};
