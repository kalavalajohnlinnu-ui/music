  ...
  try {
    ...
    if (method === 'POST' && (pathname === '/api/admin/restore-data' || pathname === '/api/admin/restore-json' || pathname === '/api/backup/restore')) {
      try {
        ...
        // Normalize students list from any JSON format
        let rawStudents = null;
        if (Array.isArray(body)) {
          rawStudents = body;
        } else if (body.tables && Array.isArray(body.tables.students)) {
          rawStudents = body.tables.students;
        } else if (Array.isArray(body.students)) {
          rawStudents = body.students;
        } else if (body.data && Array.isArray(body.data.students)) {
          rawStudents = body.data.students;
        }

        if (!rawStudents || rawStudents.length === 0) {
          return sendError(res, 400, 'Invalid backup file: No student records found in JSON');
        }

        let rawAttendance = (body.tables && Array.isArray(body.tables.attendance))
          ? body.tables.attendance
          : (Array.isArray(body.attendance) ? body.attendance : []);

        let rawBookings = (body.tables && Array.isArray(body.tables.bookings))
        ...
        try {
          const insStu = db.prepare(`
            INSERT OR REPLACE INTO students (
              id, name, mobile, photo, batch_id, custom_days, time_slot, duration_hours,
              slot_type, student_type, group_id, group_name, group_members, instruments,
              skill_level, current_lesson, homework, timezone, country, is_archived, archived_at, created_at
            ) VALUES (
              ?, ?, ?, ?, ?, ?, ?, ?,
              ?, ?, ?, ?, ?, ?,
              ?, ?, ?, ?, ?, ?, ?, ?
          ...
          rawStudents.forEach(s => {
            const sid = s.id || uid();
            const sname = (s.name || '').trim() || 'Student';
            const smob = s.mobile || '';
            const sphoto = s.photo || '';
            const sbatch = s.batchId || s.batch_id || 'batchA';
            const scustom = s.customDays ? JSON.stringify(s.customDays) : (s.custom_days || null);
            const stime = s.time || s.time_slot || '05:00';
            const sdur = parseInt(s.durationHours || s.duration_hours) || 1;
            const stype = s.slotType || s.slot_type || 'solo';
            const sformat = s.studentType || s.student_type || 'regular';
            const sgid = s.groupId || s.group_id || null;
            const sgname = s.groupName || s.group_name || null;
            const sgmems = typeof s.groupMembers === 'object' ? JSON.stringify(s.groupMembers) : (s.group_members || '[]');
            const sinsts = typeof s.instruments === 'object' ? JSON.stringify(s.instruments) : (s.instruments || '["Music"]');
            const slevel = s.skillLevel || s.skill_level || 'Beginner';
            const slesson = s.currentLesson || s.current_lesson || '';
            const shomework = s.homework || '';
            const stz = s.timezone || 'Asia/Kolkata';
            const scountry = s.country || 'India';
            const sarchived = (s.isArchived !== undefined) ? (s.isArchived ? 1 : 0) : ((s.is_archived !== undefined) ? s.is_archived : 0);
            const sarchivedAt = s.archivedAt || s.archived_at || 0;
            const screated = s.createdAt || s.created_at || Date.now();

            insStu.run(
              sid, sname, smob, sphoto, sbatch, scustom, stime, sdur,
              stype, sformat, sgid, sgname, sgmems, sinsts,
              slevel, slesson, shomework, stz, scountry, sarchived, sarchivedAt, screated
            );
          });
          ...
          if (rawBookings.length > 0) {
            const insBk = db.prepare(`
              INSERT OR REPLACE INTO bookings (
                id, name, mobile, photo, date_key, time_slot, duration_hours, slot_type, group_members, instruments, student_id, is_out_of_batch, created_at
              ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            `);
            rawBookings.forEach(b => {
              insBk.run(
                b.id || uid(), b.name, b.mobile || '', b.photo || '',
                b.dateKey || b.date_key, b.time || b.time_slot,
                parseInt(b.durationHours || b.duration_hours) || 1,
                b.slotType || b.slot_type || 'solo',
                typeof b.groupMembers === 'object' ? JSON.stringify(b.groupMembers) : (b.group_members || '[]'),
                typeof b.instruments === 'object' ? JSON.stringify(b.instruments) : (b.instruments || '["Music"]'),
        ...
        logActivity({
          actorType: 'coach',
          actorName: 'Thomas Sir',
          actionType: 'database_restored',
          title: '🗄️ JSON Database Restored',
          message: `Successfully imported ${rawStudents.length} students & ${rawAttendance.length} attendance records from backup.`,
          details: `Imported ${rawStudents.length} profiles`
        });

        return sendJson(res, 200, { success: true, message: `Successfully restored ${rawStudents.length} students into your academy roster!` });
      } catch (err) {
