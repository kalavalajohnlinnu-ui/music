...
const DEFAULT_TEMPLATE = "Hi {name} 🎵 Reminder: you have your music class at International Thomas Music Academy today at {time}. Please come on time!";
  ...
  for (const st of liveClassStudents) {
    ...
    if (isUnmarked) {
      ...
      if (!alreadySentStuLive) {
        await dispatchPush({ role: 'student', id: st.id, mobile: st.mobile, name: st.name }, {
          title: `⏰ Music Class Started: ${fmtTime(st.time_slot)}`,
          body: `Your class with Music Sir has started! Please confirm if you are attending.`,
          icon: '/icon-192.png',
          badge: '/icon-192.png',
    ...
    if (method === 'POST' && pathname === '/api/settings/reset') {
      ...
      try {
        ...
        const emergencyBackup = {
          version: '2.0',
          studio: 'International Thomas Music Academy',
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`💎 ITMA Sky Blue & Ivory Luxury Portal running on port ${PORT} (http://localhost:${PORT})`);
});
...
goldServer.listen(GOLD_PORT, '0.0.0.0', () => {
  console.log(`👑 ITMA Royal Gold & Ivory Edition running on port ${GOLD_PORT} (http://localhost:${GOLD_PORT})`);
});
...
spatialServer.listen(SPATIAL_PORT, '0.0.0.0', () => {
  console.log(`🌌 ITMA Spatial UI Portal running on port ${SPATIAL_PORT} (http://localhost:${SPATIAL_PORT})`);
});
