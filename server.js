...
try {
  ...
  if (!tInfo.includes('photo')) db.exec("ALTER TABLE students ADD COLUMN photo TEXT");
  ...
  if (!rInfo.includes('photo')) db.exec("ALTER TABLE requests ADD COLUMN photo TEXT");
  if (!rInfo.includes('note')) db.exec("ALTER TABLE requests ADD COLUMN note TEXT");
  ...
  if (!bInfo.includes('photo')) db.exec("ALTER TABLE bookings ADD COLUMN photo TEXT");
...
// 5 AM to 11 PM standard 1-hour slots (05:00 to 23:00)
...
const DEFAULT_BATCHES = [
  { id: 'batchA', name: 'Monday Batch (Mon · Wed · Fri)', sub: 'Mon, Wed, Fri', days: [1, 3, 5] },
  { id: 'batchB', name: 'Tuesday Batch (Tue · Thu · Sat)', sub: 'Tue, Thu, Sat', days: [2, 4, 6] }
];

const DEFAULT_TEMPLATE = "Hi {name} 🎵 Reminder: you have your music class at Thomas International Music Academy today at {time}. Please come on time!";
...
// -------------------------------------------------------------
// REAL-TIME LIVE SYNCHRONIZATION ENGINE (SSE + LIVE BROADCAST)
// -------------------------------------------------------------
const sseClients = new Set();
let lastDataChangeTimestamp = Date.now();

function broadcastLiveEvent(eventType, payload = {}) {
  lastDataChangeTimestamp = Date.now();
  const dataStr = JSON.stringify({ event: eventType, timestamp: lastDataChangeTimestamp, ...payload });
...
function logActivity({ actorType, actorName, actorMobile, actionType, title, message, details }) {
  ...
  broadcastLiveEvent(actionType || 'activity_logged', { title, message, actorType, actorName, actorMobile });
  return id;
}
...
function seedInitialData() {
  ...
  if (!getCfg.get('slots')) setCfg.run('slots', JSON.stringify(DEFAULT_HOURLY_SLOTS));
  if (!getCfg.get('template')) setCfg.run('template', DEFAULT_TEMPLATE);
  if (!getCfg.get('instruments')) setCfg.run('instruments', JSON.stringify(DEFAULT_INSTRUMENTS));
  if (!getCfg.get('feePlans')) setCfg.run('feePlans', JSON.stringify(DEFAULT_FEE_PLANS));
  
  if (!getCfg.get('coachMobile') || !getCfg.get('coachMobile').value) {
    setCfg.run('coachMobile', '9848173025');
  }

  // Seed initial Coach profile ONLY if not already created
  const coach = db.prepare('SELECT id FROM coaches LIMIT 1').get();
  if (!coach) {
    const { hash, salt } = hashPin('1990');
    db.prepare('INSERT INTO coaches (id, name, pin_hash, salt, mobile, created_at) VALUES (?, ?, ?, ?, ?, ?)')
      .run('coach-1', 'Thomas Sir', hash, salt, '9848173025', Date.now());
  }
}
...
function getConfig() {
    ...
    try {
      ...
      if (parsed !== null && parsed !== undefined) {
        if (Array.isArray(parsed) && parsed.length === 0) {
          if (r.key === 'slots') cfg.slots = DEFAULT_HOURLY_SLOTS;
          else if (r.key === 'batches') cfg.batches = DEFAULT_BATCHES;
          else if (r.key === 'instruments') cfg.instruments = DEFAULT_INSTRUMENTS;
          else cfg[r.key] = parsed;
        } else {
          cfg[r.key] = parsed;
        }
      }
  ...
  if (!cfg.slots || cfg.slots.length === 0) cfg.slots = DEFAULT_HOURLY_SLOTS;
  if (!cfg.batches || cfg.batches.length === 0) cfg.batches = DEFAULT_BATCHES;
  if (!cfg.instruments || cfg.instruments.length === 0) cfg.instruments = DEFAULT_INSTRUMENTS;
  if (!cfg.template || !cfg.template.trim()) cfg.template = DEFAULT_TEMPLATE;
  return cfg;
}
...
async function runNotificationEngine(overrideDate) {
  ...
  // -------------------------------------------------------------
  // FEATURE 2A: EXACT-TIME CLASS START ALARM (With Present/Absent Actions)
  // -------------------------------------------------------------
  for (const st of students) {
    if (!activeBatchesToday.includes(st.batch_id)) continue;
    if (st.time_slot === currentSlotTime) {
      const attStatus = attMap.get(st.id);

      // If student hasn't marked attendance yet
      if (!attStatus && currentMinute < 10) {
        const alreadySentAlarm = db.prepare('SELECT * FROM sent_notifications WHERE type = ? AND target_id = ? AND date_key = ? AND slot_time = ?')
          .get('class_start_alarm', st.id, todayKey, currentSlotTime);

        if (!alreadySentAlarm) {
          await dispatchPush({ role: 'student', id: st.id, mobile: st.mobile, name: st.name }, {
            title: `🔔 Class Starting Now: Are you coming? (${fmtTime(st.time_slot)})`,
            body: `Your ${safeJsonParse(st.instruments, ['Music'])[0] || 'Music'} class starts right now. Tap Present or Absent below!`,
            icon: '/icon-192.png',
            badge: '/icon-192.png',
            tag: `class-start-${st.id}-${todayKey}-${st.time_slot}`,
            ...
            data: {
              url: '/?tab=attendance&date=' + todayKey,
              dateKey: todayKey,
              studentId: st.id,
              studentName: st.name,
              slotTime: st.time_slot
            }
          }, { type: 'class_start_alarm', dateKey: todayKey, slotTime: currentSlotTime });
        }
      }

      // -------------------------------------------------------------
      // FEATURE 2B: 10-MINUTE NO-RESPONSE AUTO-ABSENT & SLOT RELEASE
      // -------------------------------------------------------------
      if (!attStatus && currentMinute >= 10) {
        const alreadyAutoAbsent = db.prepare('SELECT * FROM sent_notifications WHERE type = ? AND target_id = ? AND date_key = ? AND slot_time = ?')
          .get('auto_absent_timeout', st.id, todayKey, currentSlotTime);

        if (!alreadyAutoAbsent) {
          // 1. Mark as absent
          db.prepare(`
            INSERT INTO attendance (date_key, student_id, status, member_statuses, updated_at)
            VALUES (?, ?, 'absent', '{}', ?)
            ON CONFLICT(date_key, student_id) DO UPDATE SET status = 'absent', updated_at = excluded.updated_at
          `).run(todayKey, st.id, Date.now());

          attMap.set(st.id, 'absent');

          // 2. Feature 1: Auto-release slot as open make-up slot
          const existingMk = db.prepare('SELECT * FROM makeup_slots WHERE date_key = ? AND time_slot = ?').get(todayKey, st.time_slot);
          if (!existingMk) {
            db.prepare(`
              INSERT INTO makeup_slots (id, date_key, time_slot, duration_hours, vacated_by_id, vacated_by_name, status, created_at)
              VALUES (?, ?, ?, ?, ?, ?, 'open', ?)
            `).run(uid(), todayKey, st.time_slot, st.duration_hours || 1, st.id, st.name, Date.now());
          }

          broadcastLiveEvent('attendance_updated', { date: todayKey, studentId: st.id, status: 'absent' });

          logActivity({
            actorType: 'system',
            actorName: 'Sur Slot Auto-Release',
            actionType: 'student_absent',
            title: `⚠️ Auto-Released Slot: ${st.name}`,
            message: `${st.name} did not check in within 10 mins of class start. ${fmtTime(st.time_slot)} has been auto-released for open make-up booking.`,
            details: `Date: ${todayKey} · Slot: ${st.time_slot}`
          });

          // 3. Notify Music Sir
          dispatchPush({ role: 'coach' }, {
            title: `⚠️ Slot Auto-Released: ${st.name}`,
            body: `${st.name} did not respond within 10 mins of ${fmtTime(st.time_slot)} start. Slot is now open for other students!`,
            icon: '/icon-192.png',
            badge: '/icon-192.png',
            tag: `auto-release-${st.id}-${todayKey}`,
            data: { url: '/?tab=schedule&date=' + todayKey }
          }, { type: 'auto_absent_timeout', dateKey: todayKey, slotTime: currentSlotTime }).catch(() => {});
        }
      }
    }
  }
  ...
  for (const st of students) {
    ...
    if (st.time_slot === targetSlotTime) {
      ...
      if (!alreadySentStu) {
        await dispatchPush({ role: 'student', id: st.id, mobile: st.mobile, name: st.name }, {
          title: `⏰ Class with Thomas Sir in 1 Hour (${fmtTime(st.time_slot)})`,
          body: `Your ${safeJsonParse(st.instruments, ['Music'])[0] || 'Music'} class with Thomas Sir starts at ${fmtTime(st.time_slot)}. Are you coming? If you cannot make it, please mark absent!`,
          icon: '/thomas-sir-square.jpg',
          badge: '/icon-192.png',
  ...
  // 1b. LIVE CLASS TIME UNMARKED ATTENDANCE ALARM & PUSH NOTIFICATION (WITH "✓ Present" / "✕ Absent" BUTTONS)
  const liveClassStudents = students.filter(st => {
    if (!activeBatchesToday.includes(st.batch_id)) return false;
    const [sh] = (st.time_slot || '00:00').split(':').map(Number);
    const dur = st.duration_hours || 1;
    return currentHour >= sh && currentHour < (sh + dur);
  });

  for (const st of liveClassStudents) {
    const attRecord = db.prepare('SELECT status FROM attendance WHERE student_id = ? AND date_key = ?').get(st.id, todayKey);
    const isUnmarked = !attRecord || (attRecord.status !== 'present' && attRecord.status !== 'absent' && attRecord.status !== 'partial');

    if (isUnmarked) {
      // Send interactive push to Coach with Present & Absent buttons
      const alreadySentCoachLive = db.prepare('SELECT * FROM sent_notifications WHERE type = ? AND date_key = ? AND slot_time = ?')
        .get('coach_live_unmarked_alarm', todayKey, st.time_slot);

      if (!alreadySentCoachLive) {
        await dispatchPush({ role: 'coach' }, {
          title: `⏰ Attendance Required: ${st.name} (${fmtTime(st.time_slot)})`,
          body: `Class has started! Tap Present or Absent to confirm attendance.`,
          icon: '/thomas-sir-square.jpg',
          badge: '/icon-192.png',
          tag: `coach-live-${todayKey}-${st.id}`,
          requireInteraction: true,
          actions: [
            { action: 'mark_present', title: '✓ Present' },
            { action: 'mark_absent', title: '✕ Absent' }
          ...
          data: {
            dateKey: todayKey,
            studentId: st.id,
            url: '/?tab=schedule&date=' + todayKey
          }
        }, { type: 'coach_live_unmarked_alarm', dateKey: todayKey, slotTime: st.time_slot });
      }

      // Send interactive push to Student with Attending & Absent buttons
      const alreadySentStuLive = db.prepare('SELECT * FROM sent_notifications WHERE type = ? AND target_id = ? AND date_key = ? AND slot_time = ?')
        .get('student_live_unmarked_alarm', st.id, todayKey, st.time_slot);

      if (!alreadySentStuLive) {
        await dispatchPush({ role: 'student', id: st.id, mobile: st.mobile, name: st.name }, {
          title: `⏰ Class Started with Thomas Sir (${fmtTime(st.time_slot)})`,
          body: `Your class with Thomas Sir has started! Please confirm if you are attending.`,
          icon: '/thomas-sir-square.jpg',
          badge: '/icon-192.png',
          tag: `student-live-${st.id}-${todayKey}`,
          requireInteraction: true,
          actions: [
            { action: 'mark_present', title: '✓ I am Attending' },
            { action: 'mark_absent', title: "✕ Can't Make It" }
          ],
          data: {
            dateKey: todayKey,
            studentId: st.id,
        ...
        }, { type: 'student_live_unmarked_alarm', dateKey: todayKey, slotTime: st.time_slot });
      }
    }
  }
  ...
  if (currentHour === 21) {
    ...
    for (const st of earlyMorningStudents) {
      ...
      if (!alreadySentNight) {
        await dispatchPush({ role: 'student', id: st.id, mobile: st.mobile, name: st.name }, {
          title: `🌙 Early Morning Class Tomorrow with Thomas Sir: ${fmtTime(st.time_slot)}`,
          body: `Good evening ${st.name}! Friendly reminder that you have your music class tomorrow at ${fmtTime(st.time_slot)} with Thomas Sir. Sleep early and see you in the morning! 🎵`,
          icon: '/thomas-sir-square.jpg',
          badge: '/icon-192.png',
  ...
  if (currentHour === 7) {
    ...
    for (const st of todayStudents) {
      ...
      if (!alreadySentMorning) {
        await dispatchPush({ role: 'student', id: st.id, mobile: st.mobile, name: st.name }, {
          title: `🎼 Music Class Today with Thomas Sir at ${fmtTime(st.time_slot)}`,
          body: `Good morning ${st.name}! You have your music class today with Thomas Sir at ${fmtTime(st.time_slot)}. ${st.homework ? 'Homework: "' + st.homework + '"' : 'Keep your notes ready!'}`,
          icon: '/thomas-sir-square.jpg',
          badge: '/icon-192.png',
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (method === 'POST' && pathname === '/api/auth/student/login') {
      ...
      // 1. If 10-digit mobile is provided, search STRICTLY by mobile (Primary Unique Key)
      if (cleanPhone.length >= 10) {
        ...
        // 2. If no valid 10-digit mobile provided, search by Name
        const matchingStudents = db.prepare('SELECT * FROM students WHERE LOWER(TRIM(name)) = LOWER(?)').all(inputName);
        if (matchingStudents.length > 1) {
          return sendError(res, 400, `Multiple students found named "${inputName}". Please enter your 10-digit registered mobile number.`);
        } else if (matchingStudents.length === 1) {
          student = matchingStudents[0];
        } else {
          student = db.prepare('SELECT * FROM students WHERE LOWER(name) LIKE ? OR LOWER(group_members) LIKE ?').get(`%${inputName.toLowerCase()}%`, `%"name":"${inputName.toLowerCase()}%`);
        }
      }
      ...
      // Check in requests table (pending join application)
      let reqRecord = null;
      if (cleanPhone.length >= 10) {
        reqRecord = db.prepare('SELECT * FROM requests WHERE mobile = ? ORDER BY created_at DESC LIMIT 1').get(cleanPhone);
      } else if (inputName) {
        reqRecord = db.prepare('SELECT * FROM requests WHERE LOWER(TRIM(name)) = LOWER(?) ORDER BY created_at DESC LIMIT 1').get(inputName);
      }
      ...
      // New student applicant (prefill both name and mobile in application)
      const finalName = inputName || 'New Student';
      session = createSession('student', null, finalName, cleanPhone || '');
      return sendJson(res, 200, {
        success: true,
        status: 'new',
        user: { role: 'student', name: finalName, mobile: cleanPhone || '' },
        token: session.token
      });
    }
      ...
      if (session.role === 'coach') {
        ...
        if (reqRecord) {
          return sendJson(res, 200, {
            ...
            role: 'student',
            status: reqRecord.status,
            user: { id: reqRecord.id, name: reqRecord.name, mobile: reqRecord.mobile, photo: reqRecord.photo || '', durationHours: reqRecord.duration_hours || 1, requestNote: reqRecord.note }
          });
        }
    ...
    if (method === 'GET' && pathname === '/api/schedule') {
      ...
      const regulars = activeBatchIds.length > 0
        ? db.prepare(`SELECT id, name, mobile, photo, batch_id as batchId, time_slot as time, duration_hours as durationHours, slot_type as slotType, group_members as groupMembers, instruments, current_lesson as currentLesson, 'regular' as type FROM students WHERE batch_id IN (${activeBatchIds.map(() => '?').join(',')})`).all(...activeBatchIds)
        : [];
    ...
    if (method === 'POST' && pathname === '/api/attendance') {
      ...
      broadcastLiveEvent('attendance_updated', { date, studentId, status });
    ...
    if (method === 'POST' && pathname === '/api/students') {
      const { name, mobile, photo, batchId, time, durationHours, slotType, studentType, groupName, groupMembers, instruments, skillLevel, currentLesson } = await parseBody(req);
      const cleanPhone = normalizePhone(mobile);
      const dur = parseInt(durationHours) || 1;
      ...
      if (membersList) {
        ...
        const insertStmt = db.prepare(`
          INSERT INTO students (id, name, mobile, photo, batch_id, time_slot, duration_hours, slot_type, student_type, group_id, group_name, group_members, instruments, skill_level, current_lesson, created_at)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `);
        ...
        return sendJson(res, 201, { success: true, message: `Enrolled ${finalGroupName} (${enrichedMembers.length} siblings) into ${batchId} at ${fmtTime(time)}`, student: { id: enrichedMembers[0].id, name: finalGroupName, mobile: cleanPhone, photo: photo || '', batchId, time, durationHours: dur } });
      } else {
        const id = uid();
        db.prepare(`
          INSERT INTO students (id, name, mobile, photo, batch_id, time_slot, duration_hours, slot_type, student_type, group_id, group_name, group_members, instruments, skill_level, current_lesson, created_at)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(id, name.trim(), cleanPhone, photo || '', batchId, time, dur, stType, sType, null, null, JSON.stringify(groupMembers || []), JSON.stringify(instruments || []), skillLevel || 'Beginner', (currentLesson || '').trim(), Date.now());

        return sendJson(res, 201, { success: true, student: { id, name: name.trim(), mobile: cleanPhone, photo: photo || '', batchId, time, durationHours: dur } });
      }
    }
    ...
    if (method === 'PUT' && pathname.startsWith('/api/students/')) {
      ...
      const { name, mobile, photo, batchId, time, durationHours, slotType, studentType, groupMembers, instruments, skillLevel, currentLesson, homework } = await parseBody(req);
      ...
      logActivity({
        ...
        title: `✏️ Updated Profile: ${newName}`,
        message: `Updated timing, instruments or details for ${newName}.`,
        details: `ID: ${id} · Batch: ${newBatchId} · Time: ${fmtTime(newTime)}`
      });

      return sendJson(res, 200, { success: true, message: `Updated student ${newName}` });
    }

    if (method === 'POST' && pathname.startsWith('/api/students/') && pathname.endsWith('/photo')) {
      const parts = pathname.split('/');
      const id = parts[3];
      ...
      if (!photo) return sendError(res, 400, 'Photo data required');

      db.prepare('UPDATE students SET photo = ? WHERE id = ?').run(photo, id);
      return sendJson(res, 200, { success: true, message: `Photo updated for ${student.name}` });
    }
    ...
    if (method === 'GET' && pathname === '/api/requests') {
      const raw = db.prepare("SELECT id, name, mobile, photo, note, duration_hours as durationHours, slot_type as slotType, group_name as groupName, group_members as groupMembers, instruments, status, created_at as createdAt FROM requests WHERE status = 'pending' ORDER BY created_at DESC").all();
    ...
    if (method === 'POST' && pathname === '/api/requests') {
      const { name, mobile, photo, note, durationHours, slotType, groupName, groupMembers, instruments } = await parseBody(req);
      const cleanPhone = normalizePhone(mobile);
      const dur = parseInt(durationHours) || 1;
      ...
      const id = uid();
      db.prepare(`
        INSERT INTO requests (id, name, mobile, photo, note, duration_hours, slot_type, group_name, group_members, instruments, status, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(id, name.trim(), cleanPhone, photo || null, (note || '').trim(), dur, slotType || 'solo', (groupName || '').trim(), JSON.stringify(groupMembers || []), JSON.stringify(instruments || []), 'pending', Date.now());
    ...
    if (method === 'POST' && pathname.startsWith('/api/requests/') && pathname.endsWith('/approve')) {
      ...
      if (isGroup && parsedMembers && parsedMembers.length > 0) {
        ...
        const insertStmt = db.prepare(`
          INSERT INTO students (id, name, mobile, batch_id, time_slot, duration_hours, slot_type, student_type, group_id, group_name, group_members, instruments, skill_level, photo, created_at)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `);
    ...
    if (method === 'POST' && (pathname === '/api/settings' || pathname === '/api/config')) {
      ...
      broadcastLiveEvent('settings_updated', { slots: body.slots });
    ...
    if (method === 'POST' && pathname === '/api/settings/reset') {
      ...
      // Auto-save emergency backup snapshot before clearing
      try {
        const students = db.prepare('SELECT * FROM students').all();
        const attendance = db.prepare('SELECT * FROM attendance').all();
        const bookings = db.prepare('SELECT * FROM bookings').all();
        const performanceLogs = db.prepare('SELECT * FROM performance_logs').all();
        const requests = db.prepare('SELECT * FROM requests').all();
        const makeupSlots = db.prepare('SELECT * FROM makeup_slots').all();
        const settings = getConfig();

        const emergencyBackup = {
          version: '2.0',
          studio: 'Thomas International Music Academy',
          note: 'Auto-saved safety backup created immediately prior to manual data reset',
        ...
        const emergencyPath = path.join(__dirname, `itma_emergency_pre_reset_${Date.now()}.json`);
        fs.writeFileSync(emergencyPath, JSON.stringify(emergencyBackup, null, 2), 'utf8');
      } catch (e) {
        console.warn('Emergency snapshot warning:', e);
      }
      ...
      logActivity({
        ...
        title: '⚠️ Studio Data Reset Executed',
        message: 'Active student roster and class bookings were reset. Safety backup auto-saved.',
        details: 'Manual Reset'
      });

      return sendJson(res, 200, { success: true, message: 'All active data reset (Safety emergency backup saved)' });
    }

    // -------------------------------------------------------------
    // BULLETPROOF ERROR-FREE BACKUP & RESTORE ENGINE
    // -------------------------------------------------------------
    if (method === 'GET' && pathname === '/api/admin/integrity') {
      try {
        ...
        const photoCount = db.prepare("SELECT COUNT(*) as c FROM students WHERE photo IS NOT NULL AND photo != ''").get().c;
        return sendJson(res, 200, {
          healthy: integrityRes && integrityRes.integrity_check === 'ok',
          integrity: integrityRes ? integrityRes.integrity_check : 'ok',
        ...
        return sendError(res, 500, 'Integrity check failed: ' + err.message);
      }
    }

    if (method === 'GET' && pathname === '/api/admin/export-data') {
      try {
        // Flush all WAL pages to disk before exporting
        ...
        const requests = db.prepare('SELECT * FROM requests').all();
        const makeupSlots = db.prepare('SELECT * FROM makeup_slots').all();
        const sentNotifications = db.prepare('SELECT * FROM sent_notifications ORDER BY sent_at DESC LIMIT 200').all();
        ...
        const exportPayload = {
          ...
          studio: 'Thomas International Music Academy',
          exportedAt: new Date().toISOString(),
          checksum: `itma_${students.length}_${attendance.length}_${Date.now()}`,
          totalStudents: students.length,
        ...
        res.writeHead(200, {
          ...
          'Content-Disposition': 'attachment; filename="itma_backup_' + new Date().toISOString().split('T')[0] + '.json"',
    ...
    if (method === 'GET' && pathname === '/api/admin/download-db') {
      try {
        if (!fs.existsSync(DB_PATH)) return sendError(res, 404, 'Database file not found');

        // Flush WAL pages to ensure full atomic snapshot
        db.exec('PRAGMA wal_checkpoint(TRUNCATE);');

        const backupTempPath = path.join(__dirname, 'itma_temp_backup.db');
        if (fs.existsSync(backupTempPath)) {
          try { fs.unlinkSync(backupTempPath); } catch(e){}
        }

        // Native SQLite atomic vacuum backup
        try {
          db.exec(`VACUUM INTO '${backupTempPath.replace(/\\/g, '/')}'`);
        } catch (e) {
          // Fallback if vacuum into is not available
          fs.copyFileSync(DB_PATH, backupTempPath);
        }

        const streamFile = fs.existsSync(backupTempPath) ? backupTempPath : DB_PATH;
        const stat = fs.statSync(streamFile);

        res.writeHead(200, {
          'Content-Type': 'application/vnd.sqlite3',
          'Content-Disposition': 'attachment; filename="itma_sir_slot_' + new Date().toISOString().split('T')[0] + '.db"',
          'Content-Length': stat.size
        });

        const fileStream = fs.createReadStream(streamFile);
        fileStream.on('end', () => {
          if (streamFile === backupTempPath) {
            try { fs.unlinkSync(backupTempPath); } catch(e){}
          }
        });
        ...
        return sendError(res, 500, 'Database backup failed: ' + err.message);
      }
    }

    if (method === 'POST' && pathname === '/api/admin/restore-json') {
      const session = getSessionFromReq(req);
      if (!session || session.role !== 'coach') return sendError(res, 403, 'Unauthorized');

      const body = await parseBody(req);
      if (!body || !Array.isArray(body.students)) {
        return sendError(res, 400, 'Invalid backup file: missing students array');
      }

      // Atomic Transaction: All or Nothing (100% Error-Free)
      db.exec('BEGIN TRANSACTION;');
      try {
        const insertStudent = db.prepare(`
          INSERT OR REPLACE INTO students (
            id, name, mobile, pin_hash, salt, batch_id, time_slot, created_at,
            slot_type, group_members, instruments, skill_level, current_lesson,
            homework, duration_hours, group_id, group_name, student_type, photo
          ) VALUES (
            ?, ?, ?, ?, ?, ?, ?, ?,
            ?, ?, ?, ?, ?,
            ?, ?, ?, ?, ?, ?
        ...
        for (const s of body.students) {
          ...
          insertStudent.run(
            String(s.id),
            String(s.name).trim(),
            normalizePhone(s.mobile || '9999999999'),
            s.pin_hash || s.pinHash || 'default_hash',
            s.salt || 'default_salt',
            s.batch_id || s.batchId || 'batchA',
            s.time_slot || s.time || '05:00',
            s.created_at || s.createdAt || new Date().toISOString(),
            s.slot_type || s.slotType || 'solo',
            typeof s.group_members === 'string' ? s.group_members : JSON.stringify(s.groupMembers || []),
            typeof s.instruments === 'string' ? s.instruments : JSON.stringify(s.instruments || s.instrumentList || []),
            s.skill_level || s.skillLevel || 'Beginner',
            s.current_lesson || s.currentLesson || '',
            s.homework || '',
            parseInt(s.duration_hours || s.durationHours || 1) || 1,
            s.group_id || s.groupId || null,
            s.group_name || s.groupName || null,
            s.student_type || s.studentType || 'regular',
            s.photo || null
          );
          restoredStudents++;
        }
        ...
        if (body.attendance && Array.isArray(body.attendance)) {
          const insertAtt = db.prepare(`INSERT OR REPLACE INTO attendance (id, student_id, date, status, updated_at) VALUES (?, ?, ?, ?, ?)`);
          for (const a of body.attendance) {
            if (a.student_id && a.date) {
              insertAtt.run(
                a.id || (a.student_id + '_' + a.date),
                a.student_id,
                a.date,
                a.status || 'present',
        ...
        if (body.performanceLogs && Array.isArray(body.performanceLogs)) {
          const insertPerf = db.prepare(`INSERT OR REPLACE INTO performance_logs (id, student_id, date, rating, lesson_notes, homework, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)`);
          for (const p of body.performanceLogs) {
            if (p.student_id && p.date) {
              insertPerf.run(
                p.id || (`perf_${p.student_id}_${p.date}`),
                p.student_id,
                p.date,
                parseInt(p.rating) || 5,
                p.lesson_notes || p.notes || '',
                p.homework || '',
        ...
        if (body.settings) {
          if (body.settings.slots && Array.isArray(body.settings.slots)) setConfigValue('slots', body.settings.slots);
          if (body.settings.batches && Array.isArray(body.settings.batches)) setConfigValue('batches', body.settings.batches);
          if (body.settings.instruments && Array.isArray(body.settings.instruments)) setConfigValue('instruments', body.settings.instruments);
        }
        ...
        logActivity({
          ...
          actorName: 'Music Sir',
          actorMobile: '9848173025',
          actionType: 'data_restored',
          title: '📥 Backup Restored Successfully',
          message: `Restored ${restoredStudents} students, attendance history, and settings from backup.`,
          details: 'JSON Restore'
        });

        broadcastLiveEvent('data_restored', { totalStudents: restoredStudents });
        return sendJson(res, 200, {
          success: true,
          message: `Successfully restored ${restoredStudents} student profiles, photos, and attendance records!`
        });
      } catch (err) {
        db.exec('ROLLBACK;');
    ...
    if (method === 'GET' && pathname === '/api/known-names') {
      ...
      const students = db.prepare('SELECT name, group_members FROM students').all();
      students.forEach(s => {
        if (s.name) namesSet.add(s.name.trim());
        const members = safeJsonParse(s.group_members, []);
        members.forEach(m => { if (m.name) namesSet.add(m.name.trim()); });
      });
      const requests = db.prepare("SELECT name FROM requests WHERE status = 'pending'").all();
      requests.forEach(r => { if (r.name) namesSet.add(r.name.trim()); });

      return sendJson(res, 200, { names: Array.from(namesSet).sort().map(name => ({ name })) });
    }

    // -------------------------------------------------------------
    // REAL-TIME LIVE SYNCHRONIZATION (SSE + BACKUP POLLING)
    // -------------------------------------------------------------
    if (method === 'GET' && pathname === '/api/sync/stream') {
      res.writeHead(200, {
      ...
      res.write(`data: ${JSON.stringify({ event: 'connected', timestamp: lastDataChangeTimestamp })}\n\n`);
      sseClients.add(res);
      req.on('close', () => sseClients.delete(res));
      return;
    }

    if (method === 'GET' && pathname === '/api/sync/poll') {
      const since = parseInt(parsedUrl.searchParams.get('since')) || 0;
      const hasChanges = lastDataChangeTimestamp > since;
      return sendJson(res, 200, {
        hasChanges,
        timestamp: lastDataChangeTimestamp
      });
    }
    ...
    if (method === 'GET') {
      ...
      } else if (pathname === '/spatial' || pathname === '/spatial/' || pathname === '/spatial.html') {
        filePath = path.join(__dirname, 'spatial.html');
      } else {
...
// Main Server on Port 3000 (Chosen Sky Blue & Ivory Luxury Edition)
server.listen(PORT, '0.0.0.0', () => {
  console.log(`💎 TIMA Sky Blue & Ivory Luxury Portal running on port ${PORT} (http://localhost:${PORT})`);
});

// Dedicated Server on Port 3001 (Royal Gold & Warm Ivory Edition)
const GOLD_PORT = process.env.GOLD_PORT || 3001;
const goldServer = http.createServer((req, res) => {
  const parsedUrl = new URL(req.url, `http://${req.headers.host || 'localhost'}`);
  if (parsedUrl.pathname === '/') {
    const goldPath = path.join(__dirname, 'gold.html');
    if (fs.existsSync(goldPath)) {
      res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
      return fs.createReadStream(goldPath).pipe(res);
    }
  }
  server.emit('request', req, res);
});
goldServer.listen(GOLD_PORT, '0.0.0.0', () => {
  console.log(`👑 TIMA Royal Gold & Ivory Edition running on port ${GOLD_PORT} (http://localhost:${GOLD_PORT})`);
});

// Dedicated Server on Port 3002 (Spatial UI - VisionOS Cosmic Glass)
const SPATIAL_PORT = process.env.SPATIAL_PORT || 3002;
const spatialServer = http.createServer((req, res) => {
  const parsedUrl = new URL(req.url, `http://${req.headers.host || 'localhost'}`);
  if (parsedUrl.pathname === '/') {
    const spatialPath = path.join(__dirname, 'spatial.html');
    if (fs.existsSync(spatialPath)) {
      res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
      return fs.createReadStream(spatialPath).pipe(res);
    }
  }
  server.emit('request', req, res);
});
spatialServer.listen(SPATIAL_PORT, '0.0.0.0', () => {
  console.log(`🌌 TIMA Spatial UI Portal running on port ${SPATIAL_PORT} (http://localhost:${SPATIAL_PORT})`);
});
