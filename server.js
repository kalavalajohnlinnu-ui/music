...
const DEFAULT_TEMPLATE = "Hi {name} 🎵 Reminder: you have your music class today at {time}. Please come on time and bring your practice book!";
  ...
  try {
    ...
    if (method === 'GET' && pathname === '/api/admin/export-data') {
      ...
      res.writeHead(200, {
        ...
        'Content-Disposition': 'attachment; filename="sur_slot_backup_' + new Date().toISOString().split('T')[0] + '.json"'
      });
    ...
    if (method === 'GET' && pathname === '/api/admin/download-db') {
      if (fs.existsSync(DB_PATH)) {
        res.writeHead(200, {
          ...
          'Content-Disposition': 'attachment; filename="sir_slot.db"'
        });
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`🎶 Music Slot Booking backend running on port ${PORT} (0.0.0.0)`);
});
