...
function seedInitialData() {
  ...
  const coachCount = db.prepare('SELECT COUNT(*) as cnt FROM coaches').get();
  if (coachCount.cnt === 0) {
    ...
    console.log('✨ Seeded default Music Sir account (PIN: 1234, Mobile: 9848173025)');
  } else {
    db.prepare("UPDATE coaches SET mobile = '9848173025' WHERE mobile = '' OR mobile IS NULL").run();
  }
}
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    // AUTHENTICATION
    // -------------------------------------------------------------
    if (method === 'POST' && pathname === '/api/auth/coach/login') {
    ...
    if (method === 'GET' && pathname === '/api/students') {
      const students = db.prepare('SELECT id, name, mobile, batch_id as batchId, time_slot as time, duration_hours as durationHours, slot_type as slotType, group_members as groupMembers, instruments, skill_level as skillLevel, current_lesson as currentLesson, homework, created_at as createdAt FROM students ORDER BY time_slot ASC, name ASC').all();
      
      const now = new Date();
      const firstDay = new Date(now.getFullYear(), now.getMonth(), 1);
      const lastDay = new Date(now.getFullYear(), now.getMonth() + 1, 0);
      const fromStr = dateToKey(firstDay);
      const toStr = dateToKey(lastDay);
      const monthName = now.toLocaleString('en-IN', { month: 'long' });

      // Monthly attendance stats (strictly for current calendar month)
      const monthlyAttStats = db.prepare(`
        SELECT student_id, 
               COUNT(*) as conducted, 
               SUM(CASE WHEN status = 'present' THEN 1 WHEN status = 'partial' THEN 0.75 ELSE 0 END) as present
        FROM attendance 
      ...
      const statsMap = {};
      for (const s of monthlyAttStats) {
        statsMap[s.student_id] = {
          conducted: s.conducted,
          present: Math.round(s.present),
          pct: s.conducted > 0 ? Math.round((s.present / s.conducted) * 100) : 0
        };
      }
    ...
    if (method === 'POST' && pathname === '/api/students') {
      const { name, mobile, batchId, time, durationHours, slotType, groupMembers, instruments, skillLevel, currentLesson } = await parseBody(req);
      const cleanPhone = normalizePhone(mobile);
      const dur = parseInt(durationHours) || 1;
      ...
      const id = uid();
      db.prepare(`
        INSERT INTO students (id, name, mobile, batch_id, time_slot, duration_hours, slot_type, group_members, instruments, skill_level, current_lesson, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(id, name.trim(), cleanPhone, batchId, time, dur, slotType || 'solo', JSON.stringify(groupMembers || []), JSON.stringify(instruments || []), skillLevel || 'Beginner', (currentLesson || '').trim(), Date.now());
    ...
    if (method === 'PUT' && pathname.startsWith('/api/students/')) {
      ...
      const { name, mobile, batchId, time, durationHours, slotType, groupMembers, instruments, skillLevel, currentLesson, homework } = await parseBody(req);
      ...
      return sendJson(res, 200, { success: true, message: `Class timings updated for ${newName}` });
    }
    ...
    if (method === 'GET' && pathname === '/api/requests') {
      const raw = db.prepare('SELECT id, name, mobile, note, duration_hours as durationHours, slot_type as slotType, group_members as groupMembers, instruments, status, created_at as createdAt FROM requests ORDER BY created_at DESC').all();
    ...
    if (method === 'POST' && pathname === '/api/requests') {
      const { name, mobile, note, durationHours, slotType, groupMembers, instruments } = await parseBody(req);
      const cleanPhone = normalizePhone(mobile);
      const dur = parseInt(durationHours) || 1;
      ...
      const id = uid();
      db.prepare(`
        INSERT INTO requests (id, name, mobile, note, duration_hours, slot_type, group_members, instruments, status, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(id, name.trim(), cleanPhone, (note || '').trim(), dur, slotType || 'solo', JSON.stringify(groupMembers || []), JSON.stringify(instruments || []), 'pending', Date.now());

      // 🔔 Instant Push Alert to Music Sir
      dispatchPush({ role: 'coach' }, {
        title: `⚡ New Booking Request: ${name.trim()}`,
        body: `${name.trim()} (${cleanPhone}) requested a class session (${(instruments && instruments.length) ? instruments.join(', ') : 'Music'}). Tap to review!`,
        icon: '/icon-192.png',
        badge: '/icon-192.png',
        tag: `new-request-${id}`,
    ...
    if (method === 'POST' && pathname.startsWith('/api/requests/') && pathname.endsWith('/approve')) {
      const parts = pathname.split('/');
      const id = parts[3];
      const { batchId, time, durationHours, slotType, groupMembers, instruments, skillLevel } = await parseBody(req);
      ...
      const studentId = uid();
      db.prepare(`
        INSERT INTO students (id, name, mobile, batch_id, time_slot, duration_hours, slot_type, group_members, instruments, skill_level, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(
        studentId,
        request.name,
        ...
        time,
        dur,
        slotType || request.slot_type || 'solo',
        groupMembers ? JSON.stringify(groupMembers) : request.group_members,
    ...
    if (method === 'POST' && pathname.startsWith('/api/requests/') && pathname.endsWith('/decline')) {
      ...
      return sendJson(res, 200, { success: true, message: 'Request declined' });
    }

    // -------------------------------------------------------------
    // SETTINGS
    // -------------------------------------------------------------
    if (method === 'GET' && pathname === '/api/settings') return sendJson(res, 200, getConfig());
...
server.listen(PORT, () => {
  console.log(`🎶 Sir Slot backend running at http://localhost:${PORT}`);
});
